...
function applyScoreCaps(scores, extraction, classification, profile) {
  ...
  const adjustedTotal = Math.max(0, Math.round((scores.total - fitPenalty) * 10) / 10);
...
function makeDecisionSupport(extraction, profile, scores, classification) {
  ...
  const scenarios = candidateCategories.map((category, index) => {
    ...
    const calculatedTotal = Math.round(
      (scores.technical * 0.3 + market * 0.3 + patentSafety * 0.2 + scores.supply * 0.2) * 10
    ) / 10;
    const total = Math.min(calculatedTotal, scores.appliedCap || 100);
...
function makeValidationStatus(extraction, classification, profile) {
  ...
  if (profile === DEFAULT_EVIDENCE_PROFILE) items.push("논문·특허 원문 근거 추가");
...
export function calculateAnalysis({ idea = "", extraction = {} }) {
  ...
  const profile =
    ...
    DEFAULT_EVIDENCE_PROFILE;
  const scores = applyScoreCaps(makeScores(resolved, profile), resolved, classification, profile);
