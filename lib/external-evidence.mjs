...
export async function searchSerpTrends(query) {
  const safeQuery = limitedText(query, 180);
  const apiKey = process.env.SERP_API_KEY;
  if (!apiKey || !safeQuery) {
    return {
      source: "SERP API",
      status: apiKey ? "검색어 없음" : "환경변수 미설정",
      query: safeQuery,
      totalResults: 0,
      snippets: [],
      detail: apiKey ? "검색어가 없어 SERP 조회를 생략" : "SERP_API_KEY 설정 시 시장·특허 트렌드 보조 검색 활성화"
    };
  }

  const url = new URL("https://serpapi.com/search.json");
  url.searchParams.set("engine", "google");
  url.searchParams.set("q", `${safeQuery} market trend patent startup`);
  ...
  try {
    const response = await fetchWithTimeout(url, SERP_TIMEOUT_MS);
    if (!response.ok) throw new Error(`SERP API ${response.status}`);
    const payload = await response.json();
    const snippets = (payload.organic_results || []).slice(0, 5).map((item) => ({
      title: plainText(item.title, 160),
      snippet: plainText(item.snippet || item.rich_snippet?.top?.detected_extensions?.join(" "), 220),
      link: limitedText(item.link, 300)
    })).filter((item) => item.title || item.snippet);
    return {
      source: "SERP API",
      status: snippets.length ? "확인" : "추가 검증 필요",
      query: safeQuery,
      totalResults: Number(payload.search_information?.total_results) || snippets.length,
      snippets,
      detail: snippets.length
        ? `시장·특허·스타트업 트렌드 검색 결과 ${snippets.length}건 표본화`
        : "검색 결과 표본을 확보하지 못함"
    };
  } catch (error) {
    return {
      ...
      totalResults: 0,
      snippets: [],
      detail: error instanceof Error ? error.message : "SERP API 연결 실패"
    };
  }
}
...
export async function collectExternalEvidence({ idea, extraction }) {
  ...
  const [csv, papers, mfds, serp] = await Promise.all([
    ...
    searchSerpTrends(query)
  ]);

  return { query, csv, papers, mfds, serp };
}
